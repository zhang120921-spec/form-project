...
app.post("/api/auth/forgot-password", async (c) => {
  ...
  db.prepare("INSERT INTO password_resets (id, user_id, token_hash, expires_at) VALUES (?, ?, ?, ?)")
    .run(uid(), user.id, tokenHash, expiresAt);

  // Return the token in the response so it can be tested (production would send via email)
...
try {
  ...
  app.get("*", (c) => {
    // Skip API/admin routes — they are already handled above
    if (c.req.path.startsWith("/api/") || c.req.path === "/admin") return;
