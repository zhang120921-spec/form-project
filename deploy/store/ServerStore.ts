...
export class ServerStore implements FormStore {
  ...
  async register(email: string, password: string, displayName: string, homeClub?: string, sgaHandicap?: number): Promise<UserProfile> {
    const data = await this.request<{ user: Record<string, unknown>; token: string }>(
      "POST",
      "/auth/register",
      { email, password, displayName, homeClub, sgaHandicap }
    );
